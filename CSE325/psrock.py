...
def round_robin(number_of_rounds, *argv):
    ...
    short_report = '-'*80+'\nWin-loss-tie report for round robin, sorted by WINS:\n\n'
    
    #Make a list of teams sorted by wins
    
    #make a copy of the overall_record in two lists, so I can change the lists
    copy_teams = []
    copy_wins = []
    for team in overall_record:
        copy_teams.append(team)
        copy_wins.append(overall_record[team][0])
    ...
    for x in copy_teams:
        ...
        last_high = 9999
        high_index = 0
        #find highest score in the list
        for team_index in copy_teams:
            if copy_wins[copy_teams.index(team_index)]>highest_score and copy_wins[copy_teams.index(team_index)] < last_high:
                highest_score = copy_wins[copy_teams.index(team_index)]
                high_team = copy_teams[copy_teams.index(team_index)]
                high_index = copy_teams.index(team_index)
        #'remove' that entry from consideration to deal with duplicate scores
        copy_wins[high_index] = 9999
        sorted_teams.append(high_team)
        last_high = highest_score
    
    #for team in overall_record:
    for team in sorted_teams:
        str_length = len(str(overall_record[team][0])+\
                        '-'+str(overall_record[team][1])+\
                        '-'+str(overall_record[team][2]))
        short_report += str(overall_record[team][0])+\
                        '-'+str(overall_record[team][1])+\
                        '-'+str(overall_record[team][2])+\
                        ' '*(16-str_length)+team+': '+strategy_names[team]+'\n'
